...
if ( ! class_exists( 'WP_Commerce_About' ) ) {
	...
	class WP_Commerce_About {
		...
		/**
		 * Singleton instance of WP_Commerce_About.
		 *
		 * @var WP_Commerce_About $instance WP_Commerce_About instance.
		 */
		private static $instance;

		/**
		 * Main WP_Commerce_About instance.
		 *
		 * @since 1.0.0
		 *
		...
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof WP_Commerce_About ) ) {
				self::$instance = new WP_Commerce_About;
		...
		public function setup_config() {
			...
			$this->logo_link     = isset( $this->config['logo_link'] ) ? $this->config['logo_link'] : 'https://wpcodethemes.com/';
			$this->tabs          = isset( $this->config['tabs'] ) ? $this->config['tabs'] : array();
			$this->notification  = isset( $this->config['notification'] ) ? $this->config['notification'] : ( '<p>' . sprintf( esc_html__( 'Welcome! Thank you for choosing %1$s! To fully take advantage of the best our theme can offer please make sure you visit our %2$swelcome page%3$s.', 'wp-commerce' ), $this->theme_name, '<a href="' . esc_url( admin_url( 'themes.php?page=' . $this->page_slug ) ) . '">', '</a>' ) . '</p><p><a href="' . esc_url( admin_url( 'themes.php?page=' . $this->page_slug ) ) . '" class="button button-primary" style="text-decoration: none;">' . sprintf( esc_html__( 'Get started with %s', 'wp-commerce' ), $this->theme_name ) . '</a></p>' );
		}
		...
		public function load_assets( $hook ) {
			...
			if ( 'appearance_page_' . $this->page_slug === $hook ) {
				...
				wp_enqueue_style( 'wp-commerce-about', get_template_directory_uri() . '/inc/theme-info/css/about.css', array(), '1.0.0' );
				wp_enqueue_script( 'wp-commerce-about', get_template_directory_uri() . '/inc/theme-info/js/about.js', array( 'jquery' ), '1.0.0' );
				$js_vars = array(
					'ajaxurl' => esc_url( admin_url( 'admin-ajax.php' ) ),
				);
				wp_localize_script( 'wp-commerce-about', 'WP-Commerce-AboutObject', $js_vars );
			}
		}
						...
						if ( ! empty( $action_value['plugin_slug'] ) ) {
							...
							switch ( $active['needs'] ) {
								...
								case 'install':
									...
									$label = esc_html__( 'Install', 'wp-commerce' );
									break;
								case 'activate':
									$class = 'activate-now button button-primary';
									$label = esc_html__( 'Activate', 'wp-commerce' );
									break;
								case 'deactivate':
									$class = 'deactivate-now button';
									$label = esc_html__( 'Deactivate', 'wp-commerce' );
									break;
							}
							?>
		...
		public function upgrade_to_pro() {
			...
			if ( isset( $upgrade_to_pro['button_link'] ) && ! empty( $upgrade_to_pro['button_link'] ) ) {
				$button_text = esc_html__( 'Upgrade to Pro', 'wp-commerce' );
